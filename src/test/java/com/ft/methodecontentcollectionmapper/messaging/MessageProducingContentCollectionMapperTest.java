...
public class MessageProducingContentCollectionMapperTest {
    ...
    @Before
    public void setUp() {
        msgProducingContentCollectionMapper = new MessageProducingContentCollectionMapper(messageBuilder, producer,
    ...
    public void thatMessageIsCreatedFromMappedContentCollection() {
        ...
        ContentCollection mappedContentCollection = new ContentCollection.Builder().withUuid(UUID.randomUUID().toString()).build();

        when(eomContentCollectionMapper.mapPackage(any(), eq("tid"), eq(lastModified))).thenReturn(mappedContentCollection);

        msgProducingContentCollectionMapper.mapPackage(buildContentCollectionEomFile(), "tid", lastModified);
    ...
    public void thatCreatedMessageIsSentToQueue() {
        ...
        when(eomContentCollectionMapper.mapPackage(any(), anyString(), any())).thenReturn(mockedContentCollection);
        when(messageBuilder.buildMessage(mockedContentCollection)).thenReturn(mockedMessage);

        msgProducingContentCollectionMapper.mapPackage(buildContentCollectionEomFile(), "tid", new Date());
