...
public class DocStoreHealthCheckTest {
    ...
    public void checkAdvancedOk() throws Exception {
        ...
        when(mockUppServiceConfiguration.getEndpointConfiguration()).thenReturn(mockedEndpointConfiguration);
        ...
        when(mockedClient.resource(URI.create("http://document-store-api:8080/__gtg"))).thenReturn(mockedWebResource);
        ...
        DocStoreHealthCheck hc = new DocStoreHealthCheck(mockHCConfiguration, mockUppServiceConfiguration, mockedClient);
    ...
    public void checkAdvancedBrokenLink() throws Exception {
        HealthcheckConfiguration hcConfiguration = new HealthcheckConfiguration("name", 2, "impact", "tech summary", "panic");
        UppServiceConfiguration mockUppServiceConfiguration = mock(UppServiceConfiguration.class);
        Client mockedClient = mock(Client.class);
        EndpointConfiguration mockedEndpointConfiguration = mock(EndpointConfiguration.class);
        when(mockUppServiceConfiguration.getEndpointConfiguration()).thenReturn(mockedEndpointConfiguration);
        ...
        when(mockedBuilder.accept(MediaType.APPLICATION_JSON_TYPE)).thenReturn(mockedBuilder);
        when(mockedBuilder.header(anyString(), anyObject())).thenReturn(mockedBuilder);
        doThrow(new RuntimeException("couldn't make the call")).when(mockedBuilder).get(ClientResponse.class);
        DocStoreHealthCheck hc = new DocStoreHealthCheck(hcConfiguration, mockUppServiceConfiguration, mockedClient);
