...
public class EomFileToContentCollectionMapperTest {
	...
	public void shouldResolveContentPlaceholderWithOriginalUuid() throws IOException {
		mockContentCollection(CONTENT_COLLECTION_UUID, CONTENT_PACKAGE_TYPE,
				new EomLinkedObject.Builder().withUuid(CPH_UUID).withType("Story")
						.withAttributes(new String(
								Files.readAllBytes(Paths.get("src/test/resources/content-placeholder-attributes.xml"))))
						.build());
		when(docStoreClient.canResolveUUID(CPH_ORIGINAL_UUID, TRANSACTION_ID)).thenReturn(true);
		ContentCollection actualContentCollection = eomContentCollectionMapper.mapPackage(eomFileContentCollection,
				TRANSACTION_ID, LAST_MODIFIED);
		...
		mockContentCollection(CONTENT_COLLECTION_UUID, CONTENT_PACKAGE_TYPE,
				new EomLinkedObject.Builder().withUuid(CPH_UUID).withType("Story")
						.withAttributes(new String(
								Files.readAllBytes(Paths.get("src/test/resources/content-placeholder-attributes.xml"))))
						.build());
		when(docStoreClient.canResolveUUID(CPH_ORIGINAL_UUID, TRANSACTION_ID)).thenThrow(new IllegalArgumentException());
		eomContentCollectionMapper.mapPackage(eomFileContentCollection, TRANSACTION_ID, LAST_MODIFIED);
	}
