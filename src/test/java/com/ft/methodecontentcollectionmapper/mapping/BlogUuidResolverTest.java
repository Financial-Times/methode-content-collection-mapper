...
public class BlogUuidResolverTest {
    ...
    public void testGoodCase() {
        ...
        when(dsApiClient.resolveUUID("http://api.ft.com/system/FT-LABS-WP-1-2", "http://blogs.ft.com/the-world/?p=2193913", "tid_1"))
                .thenReturn("25966536-334f-42f0-8d57-a90bfc358c75");
        BlogUuidResolver resolver = new BlogUuidResolver(mockedMetrics, dsApiClient, "http://api.ft.com/system/", BRAND_MAPPINGS);
        String actualUuid = resolver.resolveUuid("http://blogs.ft.com/the-world?p=2193913", "2193913", "tid_1");
    ...
    public void testNoMappingFound() {
        ...
        when(mockedTimer.time()).thenReturn(mockedTimerContext);
        when(mockedTimerContext.stop()).thenReturn(0L);
        BlogUuidResolver resolver = new BlogUuidResolver(mockedMetrics, null, "http://api.ft.com/system/", BRAND_MAPPINGS);
    ...
    public void testThrowIfClientThrows() {
        ...
        DocumentStoreApiClient dsApiClient = mock(DocumentStoreApiClient.class);
        when(dsApiClient.resolveUUID("http://api.ft.com/system/FT-LABS-WP-1-2", "http://blogs.ft.com/the-world/?p=2193913", "tid_1"))
                .thenThrow(new UuidResolverException("Couldn't find, could be a 404, could be a 503, could be a network error."));
        BlogUuidResolver resolver = new BlogUuidResolver(mockedMetrics, dsApiClient, "http://api.ft.com/system/", BRAND_MAPPINGS);
    ...
    public void testThrowIfClientThrowsTransientUuidResolverException() {
        ...
        DocumentStoreApiClient dsApiClient = mock(DocumentStoreApiClient.class);
        when(dsApiClient.resolveUUID("http://api.ft.com/system/FT-LABS-WP-1-2", "http://blogs.ft.com/the-world/?p=2193913", "tid_1"))
                .thenThrow(new TransientUuidResolverException("Just some other type of exception", URI.create("document-store-api:8080/content-query?identifierAuthority=http://api.ft.com/system/FT-LABS-WP-1-2&identifierValue=2193913"), "2193913"));
        BlogUuidResolver resolver = new BlogUuidResolver(mockedMetrics, dsApiClient, "http://api.ft.com/system/", BRAND_MAPPINGS);
        resolver.resolveUuid("http://blogs.ft.com/the-world?p=2193913", "2193913", "tid_1");
    }
}
