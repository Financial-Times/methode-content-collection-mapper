...
public class MethodeContentCollectionResourceTest {
  ...
  @Mock private MessageProducingContentCollectionMapper mockMsgProducingContentCollectionMapper;
  @Mock private EomFileToContentCollectionMapper mockEomContentCollectionMapper;
  @Mock private ContentCollectionValidator mockContentCollectionValidator;
  @Mock private HttpHeaders mockHttpHeaders;
  @Mock private EomFile eomfile;
  @Mock private ContentCollection mockContentCollection;

  @Before
  public void setUp() {
    resource =
        new MethodeContentCollectionResource(
            mockMsgProducingContentCollectionMapper,
            mockEomContentCollectionMapper,
  ...
  public void thatForTransformationException500IsReturned() {
    exceptionIsThrownAndStatusCodeIsExpected(
        new TransformationException(new Exception(MSG)), HttpStatus.SC_INTERNAL_SERVER_ERROR);
  }

  @Test
  public void thatForUnsupportedTypeException422IsReturned() {
    exceptionIsThrownAndStatusCodeIsExpected(
        new UnsupportedTypeException(UUID, "", ""), HttpStatus.SC_UNPROCESSABLE_ENTITY);
  }

  private <T extends Exception> void exceptionIsThrownAndStatusCodeIsExpected(
      T exception, int status) {
    try {
      when(mockEomContentCollectionMapper.mapPackage(eq(eomfile), anyString(), Matchers.any()))
          .thenThrow(exception);
