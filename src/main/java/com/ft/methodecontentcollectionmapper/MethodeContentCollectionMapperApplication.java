...
public class MethodeContentCollectionMapperApplication
    extends Application<MethodeContentCollectionMapperConfiguration> {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(MethodeContentCollectionMapperApplication.class);
  ...
  @Override
  public void run(
      final MethodeContentCollectionMapperConfiguration configuration,
    ...
    registerDocStoreHealthCheck(
        environment, configuration.getDocumentStoreApiConfiguration(), docStoreClient);

    EomFileToContentCollectionMapper eomContentCollectionMapper =
        new EomFileToContentCollectionMapper(documentStoreApiClient, blogUuidResolver);
    ...
    MessageListener listener =
        new NativeCmsPublicationEventsListener(
            ...
            contentCollectionValidator,
            consumerConfig.getSystemCode());

    registerListener(
        environment, listener, consumerConfig, getConsumerClient(environment, consumerConfig));

    environment
        .jersey()
        .register(
            new MethodeContentCollectionResource(
                msgProducingContentCollectionMapper,
                eomContentCollectionMapper,
                contentCollectionValidator));
  ...
  private MessageBuilder getMessageBuilder(
      MethodeContentCollectionMapperConfiguration configuration, Environment environment) {
    return new MessageBuilder(
        configuration.getContentUriPrefix(),
  ...
  private Client getConsumerClient(Environment environment, ConsumerConfiguration config) {
    ...
    return ResilientClientBuilder.in(environment)
        .using(jerseyConfig)
        .usingDNS()
        .named("content-collection-consumer-client")
        .build();
  }

  private Client createDocumentStoreClient(
      Environment environment, UppServiceConfiguration config) {
    JerseyClientConfiguration jerseyConfig =
        config.getEndpointConfiguration().getJerseyClientConfiguration();
    jerseyConfig.setGzipEnabled(false);
    jerseyConfig.setGzipEnabledForRequests(false);
    return ResilientClientBuilder.in(environment)
        .using(jerseyConfig)
        .usingDNS()
        .named("content-collection-document-store-api-client")
        .build();
  }

  private MessageProducer configureMessageProducer(
      ProducerConfiguration producerConfiguration, Environment environment) {
    ...
    Client producerClient =
        ResilientClientBuilder.in(environment)
            .using(jerseyConfig)
            .usingDNS()
            .named("content-collection-producer-client")
            .build();

    final QueueProxyProducer.BuildNeeded queueProxyBuilder =
        QueueProxyProducer.builder()
            .withJerseyClient(producerClient)
            .withQueueProxyConfiguration(
                producerConfiguration.getMessageQueueProducerConfiguration());
  ...
  private void registerDocStoreHealthCheck(
      Environment environment, UppServiceConfiguration config, Client httpClient) {
    AdvancedHealthCheck healthCheck =
        new DocStoreHealthCheck(config.getHealthcheckConfiguration(), config, httpClient);
    environment
        .healthChecks()
        .register(config.getHealthcheckConfiguration().getName(), healthCheck);
  }
}
