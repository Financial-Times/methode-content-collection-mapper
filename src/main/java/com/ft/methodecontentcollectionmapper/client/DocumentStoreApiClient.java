...
public class DocumentStoreApiClient extends UppServiceClient {
  ...
  public DocumentStoreApiClient(
      Client documentStoreJerseyClient,
      String docStoreHost,
      int docStorePort,
      String docStoreHostHeader) {
    super(documentStoreJerseyClient, docStoreHost, docStorePort, null, docStoreHostHeader);
    queryEndpoint =
        UriBuilder.fromPath(QUERY_PATH).scheme("http").host(apiHost).port(apiPort).build();
    contentEndpoint =
        UriBuilder.fromPath(CONTENT_PATH).scheme("http").host(apiHost).port(apiPort).build();
    configureJersey();
  }

  private void configureJersey() {
    // Hack to force http client to stop handling redirects. This needs to be changed to the new 'DW
    // way' when we upgrade from v0.7.1
  ...
  public String resolveUUID(
      final String identifierAuthority, final String identifierValue, final String transactionId) {
    if (identifierAuthority == null || identifierValue == null) {
      throw new UuidResolverException(
          "Neither the identifierAuthority nor identifierValue should be null!");
    }

    final URI queryUri;
    try {
      queryUri =
          UriBuilder.fromUri(queryEndpoint)
    ...
    return processResponse(
        ...
        resp -> {
          if (resp.getStatus() == HttpStatus.SC_NOT_FOUND) {
            throw new TransientUuidResolverException(
                String.format(
                    "uuid not found in document-store-api status=%d queryURI=%s",
                    resp.getStatus(), queryUri),
                queryUri,
                identifierValue);
          }
          if (resp.getStatus() != HttpStatus.SC_MOVED_PERMANENTLY) {
            throw new TransientUuidResolverException(
                String.format(
                    "unexpected status code from document-store-api status=%d queryURI=%s",
                    resp.getStatus(), queryUri),
                queryUri,
                identifierValue);
          }
          ...
          if (redirectUrl == null) {
            LOG.error(
                "DS API could not find the required resource! DS Query URL [{}], Response [{}]",
                queryUri.toString(),
                resp.toString());
            throw new UuidResolverException(
                "DS API could not find the required resource! DS Query URL ["
                    + queryUri.toString()
                    + "], Response ["
                    + resp.toString()
                    + "]");
  ...
  public boolean canResolveUUID(String uuid, String transactionId) {
    ...
    switch (response.getStatus()) {
      ...
      default:
        throw new UuidResolverException(
            String.format("Cannot resolve uuid: %s in DocumentStore", uuid));
    }
  }
