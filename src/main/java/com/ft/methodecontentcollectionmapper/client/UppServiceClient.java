...
public abstract class UppServiceClient {
  ...
  public UppServiceClient(
      final UppServiceConfiguration uppServiceConfiguration, final Environment environment) {
    EndpointConfiguration endpointConfiguration =
        uppServiceConfiguration.getEndpointConfiguration();
    jerseyClient =
        ResilientClientBuilder.in(environment)
            ...
            .withContinuationPolicy(
                new ExponentialBackoffContinuationPolicy(
                    uppServiceConfiguration.getNumberOfConnectionAttempts(),
                    uppServiceConfiguration.getTimeoutMultiplier()))
            .build();
  ...
  public UppServiceClient(
      Client jerseyClient, String apiHost, int apiPort, String apiPath, String hostHeader) {
  ...
  protected <R> R processResponse(
      final ClientResponse response, final Function<ClientResponse, R> processor) {
    try {
