...
public class NativeCmsPublicationEventsListener implements MessageListener {
  ...
  public NativeCmsPublicationEventsListener(
      MessageProducingContentCollectionMapper messageProducingContentCollectionMapper,
  ...
  private void handleMessage(Message message, String transactionId) {
    try {
      EomFile methodeContent =
          objectMapper.reader(EomFile.class).readValue(message.getMessageBody());
      contentCollectionValidator.validate(methodeContent);
      messageProducingContentCollectionMapper.mapPackage(
          methodeContent, transactionId, message.getMessageTimestamp());
