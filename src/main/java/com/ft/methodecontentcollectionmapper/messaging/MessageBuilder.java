...
public class MessageBuilder {
  ...
  public Message buildMessage(ContentCollection contentCollection) {
    String specificContentUriPrefix =
        contentUriPrefix + contentCollection.getType().getContentUriSuffix();
    UriBuilder contentUriBuilder = UriBuilder.fromUri(specificContentUriPrefix).path("{uuid}");

    MessageBody msgBody =
        new MessageBody(
            contentCollection,
            contentUriBuilder.build(contentCollection.getUuid()).toString(),
            RFC3339_FMT.format(
                OffsetDateTime.ofInstant(contentCollection.getLastModified().toInstant(), UTC)),
  ...
  public static class MessageBody {
    ...
    MessageBody(
        @JsonProperty("payload") ContentCollection payload,
