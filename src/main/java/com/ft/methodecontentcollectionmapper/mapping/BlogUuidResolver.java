...
public class BlogUuidResolver {
  ...
  public BlogUuidResolver(
      MetricRegistry metrics,
      DocumentStoreApiClient client,
      String authorityPrefix,
      Map<String, String> brandMappings) {
    this.client = client;
  ...
  public String resolveUuid(String guid, String postId, String transactionId) {
    ...
    try {
      ...
      throw new UuidResolverException(
          "No brand mapping for Blog serviceId [" + guid + "] - mapping [" + mappingKey + "]!");
    } finally {
      context.stop();
    }
  }
  ...
  private String resolve(URI guidUri, String postId, String key, String transactionId) {
    ...
    String identifierValue =
        guidUri.getScheme()
            ...
            + key
            + "/?p="
            + postId; // Wordpress article transformer homogenises the identifierValue, so this is
    // ok to do.
