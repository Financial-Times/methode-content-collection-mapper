...
public class DocStoreHealthCheck extends AdvancedHealthCheck {
  ...
  public DocStoreHealthCheck(
    ...
    contentReadHeathCheckUri =
        UriBuilder.fromPath("/__gtg").scheme("http").host(host).port(port).build();
  ...
  protected AdvancedResult checkAdvanced() {
    ...
    try {
      WebResource.Builder webResourceBuilder =
          client.resource(contentReadHeathCheckUri).getRequestBuilder();
