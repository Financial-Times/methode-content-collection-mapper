package com.ft.methodestorypackagemapper.health;
...
public class CanConnectToMessageQueueProducerProxyHealthcheck extends AdvancedHealthCheck {
    ...
    public CanConnectToMessageQueueProducerProxyHealthcheck(QueueProxyHealthcheck queueProxyHealthcheck,
            HealthcheckConfiguration healthcheckConfiguration, final MetricRegistry metrics) {
        super(healthcheckConfiguration.getName());
        this.queueProxyHealthcheck = queueProxyHealthcheck;
        this.healthcheckConfiguration = healthcheckConfiguration;
        if (metrics != null) {
            timer = metrics.timer(
                    MetricRegistry.name(CanConnectToMessageQueueProducerProxyHealthcheck.class, "checkAdvanced"));
        } else {
            timer = new Timer();
        }
    }
